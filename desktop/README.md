...
```bash
sudo dpkg -i neriplayer_1.4.6-1_amd64.deb
...
- **托盘菜单（桌面原生渲染）**：优先注册 **StatusNotifierItem + DBusMenu**（GNOME / KDE / XFCE 等
  现代桌面都支持）——菜单由桌面自己绘制，点菜单项通过 D-Bus 回调应用，因此**点击一定生效**；
  菜单包含播放 / 暂停、上一首、下一首（勾选式）悬浮歌词开关、显示主窗口、下载管理、设置、
  打开控制面板、退出；菜单文案随播放状态实时刷新。
  桌面不支持时装回退方案（AWT 托盘图标 + 应用内自绘面板）。
...
> 为什么不直接用 `java.awt.TrayIcon`：GNOME（Ubuntu 默认桌面）里 AWT 图标走的是 XEmbed 兼容层，
> 点图标时 GNOME 会一边把点击转发给 XEmbed 窗口、一边自己握住输入抓取；应用这时再弹自己的窗口，
> 用户的点击就被抓取吃掉了——表现正是「菜单弹出来了，但按钮点不动」。改走桌面原生的
> StatusNotifierItem + DBusMenu 后，菜单由桌面渲染，点击通过 D-Bus 回调应用，不存在这类冲突；
> 应用内面板仍然保留（菜单里的「打开控制面板」），它是应用自己绘制的窗口，配色与主界面完全一致。
